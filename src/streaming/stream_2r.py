...
def build_cfg_radar(cfg_params, mid_gap_m=0.3, beam_width_deg=40.0, beam_center_deg=90.0, exp_name="test", save_raw_dt=False, doppler=False, max_range_m=None):
    """
    ...
    #   Range selection                          (Windows Implementation Refactor)
    #   ---------------     range-gates the realtime rat detector to a short range
    #
    #                       NOTE :  project_3 Implementation keeps the default full map, but --max-range-m can enable the
    #                               same close-range behavior while still projecting into the global map.
    ...
    if max_range_m is None:
        ...
        max_bin = max(8, int(float(max_range_m) / range_resolution))
        r_idxs  = np.arange(0, min(max_bin, num_range_bins), dtype=int)
    ...
    # Radar poses
    # -----------
    radar1_pose, radar2_pose = make_radar_poses(y_mid_gap=mid_gap_m, yaw1_deg=0.0, yaw2_deg=0.0)
    ...
    cart_x_max_m    = float(range_bins_m[-1]) if max_range_m is not None else cfg_params["MAX_RANGE"]

    cart_map_max_m  = cart_x_max_m
    cart_y_min_m    = -cart_map_max_m - padding_y  # y: right side + radar offset
    cart_y_max_m    =  cart_map_max_m + padding_y  # y: left side + radar offset
    ...
    cfg_radar = {
        ...
        # Multi-radar geometry
        "n_radar"            : 2,                                   # Number of radars
        "dist_between_radars": mid_gap_m,                           # Radar mid_gap [m]
        "radar1_pose"        : radar1_pose,                         # Radar 1 pose: (x, y, yaw_deg)
        "radar2_pose"        : radar2_pose,                         # Radar 2 pose: (x, y, yaw_deg)
        ...
        # WINDOWS IMPLEMENTATION -- realtime detection defaults
        "detector_range_start_bin"     : 15,
        "detector_range_stop_bin"      : 40,
        "detector_sensitivity"         : 1.0,
        "detector_min_active_bins"     : 1,
        "detector_frame_window"        : 10,
        "detector_frame_threshold"     : 0.25,
        "cart_detector_threshold"      : 0.35,
        "cart_detector_ema_alpha"      : 0.5,
        "cart_detector_marker_min_amp" : 0.25,
        "detector_cart_roi_m"          : None,
        "display_exponent"             : 1,
        "record_dataset"               : True,
        "record_mp4"                   : True,
...
def build_cfg_cfar(cfar_on=False, doppler=False, bg_sub=None, doppler_notch_bins=1, doppler_snr_threshold=2.0, doppler_noise_multiplier=5.0, near_range_zero_bins=5, far_range_zero_bins=0):
    """
    ...
    return {
        "cfar_on"                   : cfar_on,
        "bg_sub"                    : bg_sub,           # WINDOWS IMPLEMENTATION : frame differencing when not using Doppler mode
        ...
        "temporal_history_frames"   : 5,
        "dense_average_frames"      : 2,
        "near_range_zero_bins"      : near_range_zero_bins,
        "far_range_zero_bins"       : far_range_zero_bins,
        "doppler_notch_bins"        : doppler_notch_bins,
        "doppler_snr_threshold"     : doppler_snr_threshold,
        "doppler_noise_multiplier"  : doppler_noise_multiplier,
        "read_timeout_s"            : 5.0,
    }
...
def print_runtime_summary(cfg_file_path, cfg_radar, cfg_cfar):
    ...
    print(f"  Distance btwn. radars:  {cfg_radar['dist_between_radars']:.3f} m")
    print(f"                 =mid_gap")
    ...
    if cfg_radar["save_raw_dt"]:
        print(f" Recording Acquired Data from Radars")
...
def main():
    ...
    parser.add_argument("--mid_gap"                     , type=float    , default=0.2       , help="Distance between the two radars in meters.")
    parser.add_argument("--beam-width"                  , type=float    , default=90.0      , help="Beamforming angular width in degrees.")
    parser.add_argument("--beam-center"                 , type=float    , default=90.0      , help="Beamformer center angle in degrees. 90° means straight ahead.")
    parser.add_argument("--max-range-m"                 , type=float    , default=None      , help="(optional) WINDOWS IMPLEMENTATION range gate in meters, e.g. 1.5 for rat detection.")
    parser.add_argument("--no-bg-sub"                   , action="store_true"               , help="Disable previous-frame subtraction in non-Doppler mode.")
    parser.add_argument("--doppler-notch-bins"          , type=int      , default=1         , help="Number of zero-velocity Doppler bins to notch on each side of DC.")
    parser.add_argument("--doppler-snr-threshold"       , type=float    , default=2.0       , help="SNR gate for choosing moving Doppler bins.")
    parser.add_argument("--doppler-noise-multiplier"    , type=float    , default=5.0       , help="Median peak-power multiplier for moving-bin masking.")
    parser.add_argument("--near_range_zero_bins"        , type=int      , default=5         , help="Removes everything closer to the specified range bin")
    parser.add_argument("--far_range_zero_bins"         , type=int      , default=0         , help="Removes everything farther to the specified range bin")
    ...
    cfg_radar = build_cfg_radar(cfg_params, mid_gap_m=args.mid_gap, beam_width_deg=args.beam_width, beam_center_deg=args.beam_center, exp_name=args.exp_name, save_raw_dt=args.save_raw_dt, doppler=args.doppler, max_range_m=args.max_range_m)
    cfg_cfar  = build_cfg_cfar(args.cfar, doppler=args.doppler, bg_sub=(False if args.no_bg_sub else None), doppler_notch_bins=args.doppler_notch_bins, doppler_snr_threshold=args.doppler_snr_threshold, doppler_noise_multiplier=args.doppler_noise_multiplier, near_range_zero_bins=args.near_range_zero_bins, far_range_zero_bins=args.far_range_zero_bins)
