...
def make_radar_poses(y_baseline, yaw1_deg=0.0, yaw2_deg=0.0):
    """
    ...
    Therefore, if the two radars are separated laterally by `y_baseline`,
    their default positions are:

        Radar 1: y = +y_baseline / 2    -- on the left side
        Radar 2: y = -y_baseline / 2    -- on the right side
    ...
    radar1_pose = (0.0,  y_baseline / 2.0, yaw1_deg)    # Left side of origin (on Y-axis)
    radar2_pose = (0.0, -y_baseline / 2.0, yaw2_deg)    # Right side of origin (on Y-axis)
...
def build_cfg_radar(cfg_params, baseline_m=0.0, beam_width_deg=40.0, beam_center_deg=90.0, exp_name="test", save_raw_dt=False, doppler=False):
    """
    ...
    #make_radar_poses(y_baseline=baseline_m, yaw1_deg=0.0, yaw2_deg=0.0)
    ...
    padding_y = baseline_m/2        # radar offset to pad sides of the grid (since otherwise each radar's max range exceeds the unpadded grid sides) 
    ...
    cfg_radar = {
        ...
        "dist_between_radars": baseline_m,                       # Radar baseline [m]
...
def print_runtime_summary(cfg_file_path, cfg_radar, cfg_cfar):
    ...
    #print(f"  Baseline:               {cfg_radar['dist_between_radars']:.3f} m")
...
def main():
    ...
    # parser.add_argument("--baseline"    , type=float , default=0.0   , help="Distance between the two radars in meters.")
    ...
    #cfg_radar = build_cfg_radar(cfg_params, baseline_m=args.baseline, beam_width_deg=args.beam_width, beam_center_deg=args.beam_center, exp_name=args.exp_name, save_raw_dt=args.save_raw_dt, doppler=args.doppler)
    cfg_radar = build_cfg_radar(cfg_params, baseline_m=0.0, beam_width_deg=args.beam_width, beam_center_deg=args.beam_center, exp_name=args.exp_name, save_raw_dt=args.save_raw_dt, doppler=args.doppler)
